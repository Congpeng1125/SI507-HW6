...
Change the code `document.querySelector('h1').innerHTML = "A name";` in `function displayInformation ()` to `document.querySelector('h1').innerHTML = "Cong Peng";` ,for this is the commend showing 'A name' on the website.
...
The reason is that there are two elements with <p></p> tags where border color is set as white #FFFFFF. Backgrounds of them are set as grey by `p{ background-color: #b3b3b3;}`
To change the color of the border, we can set the color of the border by `border: 3px solid #b3b3b3;`. To change the color of the background, we can set by `background-color: #b3b3b3;`.
